        ...
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages.map((page) => (
              ...
              >
               <NavLink to={`/${page}`} style={{color:"white"}} >{page}</NavLink>
              </Button>
            ))}
          </Box>
          ...
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                {/* { isUser ? <>{userName}</> : <></>} */}
                { isUser ? <Avatar alt={userName} src={userImage} />: <Avatar alt="?" src="https://play-lh.googleusercontent.com/7Ac5TgaL15Ra4bvFVHJKCdJp4qvnL4djZj5bKc6RN-MZjzrvkeHbJytek0NPTSdZcp8" />}
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: '45px' }}
